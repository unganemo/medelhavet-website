import about from '../../images/about.png'
import store from '../../images/store.png'
import garden from '../../images/garden.png'

export const aboutObj = {
    id: 'about',
    lightBg: true,
    title: 'om oss',
    description: 'En liten bit av Medelhavet är en trädgård och butik i Hammenhög på Österlen som drivs av familjen Karakitsos. Resan med En liten bit av Medelhavet började för 16 år sedan då vi köpte fastigheten där vår butik idag huserar. Då var trädgården bortglömd och vildvuxen och väldigt olik den trädgård vi har idag. Första sommaren vigdes åt underarbete. Vi plockade sten, kirskål och fällde självsådda träd – därefter fick vi kväva resterande ogräs innan vi kunde lägga in ny jord. Efterföljande år fortsatte arbetet med att anlägga trädgården. Då Vasilis är från Grekland fanns redan tidigt en tanke om att i trädgården driva upp växter från Medelhavet. Samma år fick Vasilis en vinranka och ett fikonträd i födelsedagspresent av Diane. Denna första vinranka är samma vinranka som idag sträcker sig långt upp över husfasad och balkong och som fick bli grunden till trädgårdens övriga vinrankor. År 2007 öppnade vi En liten bit av Medelhavet, butik och visningsträdgård och sedan dess har vår trädgård och butik vuxit organiskt.',
    imgStart: true,
    img: about,
    alt: 'Om oss',
    darkText: true
}

export const storeObj = {
    id: 'store',
    lightBg: false,
    title: 'butiken',
    description: 'Om du söker dofter och smaker från Medelhavet, är detta butiken för dig! Hos oss finner du ett stort utbud av grekiska delikatesser såsom ostar, oliver och olivoljor från både fastlandet och övärlden. I butiken finns även vår egendesignade handgjorda keramik – också den från Grekland! Vi har även ett stort sortiment av frosttåliga handgjorda terrakottakrukor, direktimporterade från Grekland.',
    imgStart: false,
    img: store,
    alt: 'Butiken',
    darkText: false
}

export const gardenObj = {
    id: 'garden',
    lightBg: true,
    title: 'trädgården',
    description: 'För att få en känsla av Medelhav har vi i trädgården vitputsade murar och vinrankor som växer och breder ut sig över pergola och balkong. Rosor, hortensia och pipört växer sida vid sida med oliv-, aprikos-, fikon-, kvitten-, mispel-, sharon-, kiwi-, och körsbärsträd. Salvia, oregano, rosmarin, timjan, kyndel, mynta och lavendel binder samman trädgården till en harmoni av färger och dofter som ger medelhavskänsla på skånskt vis. Trädgården är omgärdad av hus och murar vilket skapar ett gynnsamt mikroklimat; under sensommar och höst kan vi skörda och njuta av druvor, kvitten och fikon. Som ung fick Vasilis erfarenhet av odling från familjens körsbär-, persiko-, vin-, kiwi-, och olivodlingar. Denna kunskap har kommit väl till pass även i Skåne och samtliga av våra träd och växter har vi drivit upp själva.',
    imgStart: true,
    img: garden,
    alt: 'Trädgården',
    darkText: true
}

export const cafeObj = {
    id: 'cafe',
    lightBg: false,
    title: 'café',
    description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut rutrum dui lorem, eu blandit felis fringilla sed. Fusce ultrices vitae ante at ultrices. Ut eleifend, dui feugiat iaculis elementum, ipsum felis scelerisque sapien, sed sodales lorem orci sit amet tortor. Vivamus lacinia nibh gravida elementum mollis. Nunc convallis mi ut finibus laoreet',
    imgStart: false,
    img: garden,
    alt: 'Café',
    darkText: false
}